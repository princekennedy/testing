<template>
    <app-layout>
        ...
        <template #actions v-if="checkRole($page.props.auth.data,'administrator')">
            <inertia-link :href="route('vehicles.create')">
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
                <div class="page-section" v-if="checkRole($page.props.auth.data,'management') || checkRole($page.props.auth.data,'administrator')">
                    ...
                    <div class="page-section-content grid grid-cols-1 md:grid-cols-2">
                        <div
                            ...
                            :key="index"
                            v-if="vehicle.verified===0"
                            @click="viewUser(vehicle.id)"
                        >
                            <div class="flex justify-between items-center">
                                <div class="">
                                    <div class="name">Registration Number: {{ vehicle.vehicleRegistrationNumber }}</div>
                                    <div class="position">{{ numberWithCommas(vehicle.mileage) }}KM</div>
                                </div>
                ...
                <div class="page-section">
                        ...
                        >
                            <div class="flex justify-between items-center">
                                ...
                                <div class=" h-10 w-10 flex justify-center items-center rounded-full bg-blue-700">
                                    <i v-if="vehicle.status==1" class="mdi mdi-check-decagram text-white"></i>
                    ...
                    <div v-if="vehicles.data.length === 0" class="text-center text-gray-400 md:col-span-2 text-sm">
                        No Vehicles Found
                    </div>
                    <div v-else>
                        <div class="flex flex-col items-center">
                            ...
                            <span class="text-sm text-gray-700 dark:text-gray-400">
                                    Showing <span class="font-semibold text-gray-900 dark:text-white">{{ vehicles.meta.from }}</span> to <span class="font-semibold text-gray-900 dark:text-white">{{ vehicles.meta.to }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ vehicles.meta.total }}</span> Vehicles
                                </span>
                            <div class="inline-flex mt-2 xs:mt-0">
                                <!-- Previous Button -->
                                <a :href="vehicles.links.prev" class="inline-flex items-center px-4 py-2 mr-3 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                    <svg aria-hidden="true" class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l2.293 2.293a1 1 0 010 1.414z" clip-rule="evenodd"></path></svg>
                                    Previous
                                </a>
                                <a :href="vehicles.links.next" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                                    Next
...
<script>
    ...
    export default {
        ...
        methods:{
            viewUser(id){
                this.$inertia.get(this.route('vehicles.show',{'id':id}))
            }
        }
