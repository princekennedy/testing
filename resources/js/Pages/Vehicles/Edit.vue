            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="heading-font uppercase text-sm font-medium text-gray-500 dark:text-gray-400">
                        {{ vehicle.data.vehicleRegistrationNumber }}
                    </span>
                </div>
            </li>
        </template>
                    ...
                    <div class="page-section-content">
                        <form @submit.prevent="submit" class="flex justify-center">
                            <div class="card w-full sm:max-w-md">
                                ...
                                <div v-if="form.photo !=='' && form.photo" class="mb-4">
...
<script>
    ...
    export default {
        ...
        data(){
          return{
              form: this.$inertia.form({
                  photo:this.vehicle.data.photo,
                  vehicleRegistrationNumber:this.vehicle.data.vehicleRegistrationNumber,
                  gasId:this.vehicle.data.gas.id,
                  mileage:this.vehicle.data.mileage,
                  lastRefillFuelReceived:this.vehicle.data.lastRefillFuelReceived,
                  lastRefillMileageCovered:this.vehicle.data.lastRefillMileageCovered,
              }),
              date:null,
        ...
        created() {
            this.date=this.vehicle.data.lastRefillDate!==null && this.vehicle.data.lastRefillDate!==0?new Date(this.vehicle.data.lastRefillDate * 1000).toISOString():null
        },
        computed:{
            lastRefillDate(){
        ...
        methods:{
            submit() {
                this.form.post(this.route('vehicles.update',{id:this.vehicle.data.id}))
            },
            photoUpload(file){
